...
public interface PostRepository extends JpaRepository<Post, Long> {
    ...
    @Query("SELECT p FROM Post p LEFT JOIN p.group g WHERE g IS NULL OR g.privacy <> com.example.system.domain.model.GroupPrivacy.WALL ORDER BY p.views DESC")
    List<Post> findTopByViews();

    @Query("SELECT p FROM Post p LEFT JOIN p.group g WHERE g IS NULL OR g.privacy <> com.example.system.domain.model.GroupPrivacy.WALL ORDER BY p.likes DESC")
    List<Post> findTopByLikes();

    @Query("SELECT p FROM Post p LEFT JOIN p.group g WHERE g IS NULL OR g.privacy <> com.example.system.domain.model.GroupPrivacy.WALL ORDER BY p.createdAt DESC")
